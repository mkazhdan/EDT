...
struct ImageReader
{
	...
	static unsigned char* ReadColor( const char* fileName , unsigned int& width , unsigned int& height )
	{
		...
		ImageReader* reader = Get( fileName , width , height , channels );
		if( channels!=1 && channels!=3 && channels!=4 ) ERROR_OUT( "Only one-, three-, or four-channel input supported: " , channels ) , exit( 0 );
		...
		for( unsigned int j=0 ; j<height ; j++ )
		{
			...
			else if( channels==1 ) for( unsigned int i=0 ; i<width ; i++ ) for( unsigned int c=0 ; c<3 ; c++ ) pixels[3*(j*width+i)+c] = pixelRow[i];
			else if( channels==4 ) for( unsigned int i=0 ; i<width ; i++ ) for( unsigned int c=0 ; c<3 ; c++ ) pixels[3*(j*width+i)+c] = pixelRow[4*i+c];
		}
		delete[] pixelRow;
		delete reader;
		return pixels;
